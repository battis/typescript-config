...
const args = cli.init({
    ...
    args: {
        options: {
            package: {
                short: 'p',
                description: `Path to package.json to examine (defaults to package in current working directory: ${cli.colors.url(path.resolve(CWD, 'package.json'))})`,
        ...
        flags: {
            ...
            write: {
                short: 'w',
                description: `Write changes to package.json (required to make changes, omitting ${cli.colors.command('--write')} is a dry run that outputs results to the console)`
            },
            optional: {
                short: 'o',
            ...
            dev: {
                short: 'd',
                description: `Install missing peers as ${cli.colors.value('devDependencies')} (defaults to true, ${cli.colors.command('--no-dev')} to install as regular ${cli.colors.value('dependencies')})`,
                default: true
            },
            metadata: {
                short: 'm',
                description: 'Update package metadata to indicate which peers have been installed by this tool (defaults to false)',
                default: false
            },
            cache: {
                short: 'c',
                description: `Update ${cli.colors.url(cacheFilename)} in the package root to indicate which peers have been installed by this tool (defaults to true, ${cli.colors.command('--no-cache')} to omit)`,
                default: true
            }
        }
    }
...
async function addToPeers({ peerName, peerVersion = '*', dependency, packageName, packageVersion = '*' }) {
    cli.log.info(`${cli.colors.value(`${peerName}@${peerVersion}`)} is peer of ${cli.colors.value(`${packageName}@${packageVersion}`)}`);
    if (peerName in peers[dependency] &&
...
async function identifyPeers({ targetPackage, includeDev = false }) {
            ...
            if (dependency != 'devDependencies' || includeDev) {
                for (const packageName in targetPackage[dependency]) {
                    ...
                    try {
                        ...
                        dependencyPackage = await fetchPackage(packageName, targetPackage[dependency][packageName]);
                    }
                    if (dependencyPackage) {
                        ...
                        spinner.fail(`${packageName}@${packageVersion} could not be satisfied (peer of ${targetPackage.name}@${targetPackage.version}`);
                    }
                }
            }
        }
...
const cachePath = path.join(path.basename(packagePath) == 'package.json'
    ? path.dirname(packagePath)
    : packagePath, cacheFilename);
...
if (fs.existsSync(cachePath)) {
    ...
    if (cache) {
        spinner.succeed(`Cache loaded from ${cli.colors.url(packagePath)}.${cli.colors.value('peerDependenciesMeta')}`);
    }
}
if (remove) {
    if (!cacheHistory) {
        ...
        if (cacheHistory[peerName].peerOf) {
            for (const dependency of ['devDependencies', 'dependencies'])
                if (projectPackage[dependency] &&
                    ...
                    cli.log.info(`${cli.colors.value(`${peerName}@${projectPackage[dependency][peerName]}`)} removed from ${cli.colors.value(dependency)}`);
    ...
    projectPackage = {
        ...
        dependencies: {
            ...
            ...(dev
                ? {}
                : Object.keys(peers).reduce((dependencies, typeOfDependency) => ({
                    ...dependencies,
...
cacheHistory = {
    ...cacheHistory,
    ...Object.keys(peersMeta).reduce((built, peer) => {
        if (!projectPackage.peerDependenciesMeta ||
...
if (cache) {
    if (write) {
        ...
        spinner.succeed(`Computed ${cli.colors.url(cachePath)}\n${JSON.stringify(cacheHistory, null, 2)}`);
    }
}
if (metadata) {
    spinner.start(`Adding cache data to ${cli.colors.value('peerDependenciesMeta')}`);
    projectPackage.peerDependenciesMeta = {
        ...projectPackage.peerDependenciesMeta,
        ...cacheHistory
    };
    spinner.succeed(`Cache data added to ${cli.colors.value('peerDependenciesMeta')}`);
}
...
if (write) {
    ...
    spinner.succeed(`Computed ${cli.colors.url(packagePath)}\n${packageContents}`);
}
